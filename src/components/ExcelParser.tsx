...
import {
	...
	convertExcelToJson,
	createExcelByJson, createTemplate,
	FileRowData,
	InvalidFileRowData,
	ValidFileRowData,
...
const ExcelParser = () => {
	...
	const props: UploadProps = {
		...
		beforeUpload: async (file: RcFile) => {
			...
			if (isSizeAndTypeMatched) {
				...
				// 表格数据格式校验
				const { total, validList, invalidList } = await batchCheckProductData(tableOriginData)
	...
	return <div className="bg-gray-100 rounded-lg mb-10 p-10">
		<button className={btnStyle} onClick={createTemplate}>下载数据模版</button>
		...
		{[ParseStatus.SUCCESS, ParseStatus.PARTIAL_SUCCESS].includes(parseStatus) &&
			<div className="border-t border-gray-300 py-10 mt-10">
				<h3 className="mb-10">{counter.total} 条数据中 {counter.valid} 条为有效数据</h3>
				{!!validList.length && <div className="bg-white rounded-xl p-10 flex flex-col gap-2">
					{validList.map(item => <p key={item.id}>id: {item.id}, amount: {item.amount}</p>)}
				</div>}
			</div>
		}

		{[ParseStatus.ERROR, ParseStatus.PARTIAL_SUCCESS].includes(parseStatus) &&
			<button className={btnStyle} onClick={exportInvalidData}>下载问题数据</button>
		}
	</div>;
};
