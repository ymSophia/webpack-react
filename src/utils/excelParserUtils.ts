...
function markDuplicateIds<T extends { id: string }, V extends T & { isDuplicated?: boolean }>(arr: T[]) {
...
export const createExcelByJson = (headerConfig: HeaderConfigItem[], dataList: any[], fileName: string) => {
  // 定义要写入Excel的数据
...
export const batchCheckProductData = async (originData: FileRowData[]) => {
  const data = originData.map((item) => ({ id: `${item.id}`.trim(), amount: `${item.amount}`.trim() }))
  ...
  markDuplicateIds(data).forEach((item) => {
    ...
    if (isAmountValid && !item.isDuplicated) {
      ...
      invalidList.push({ id: item.id, amount: item.amount, hint: hintList.join("; ") })
    }
  })
